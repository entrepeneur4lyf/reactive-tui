                ...
                Ok(true) => {
                    match event::read() {
                        ...
                        Ok(Event::FocusGained) => {
                            let custom_event =
                                serde_json::json!({"type": "focus_gained", "platform": "windows"});
                            if event_sender
                        ...
                        Ok(Event::FocusLost) => {
                            let custom_event =
                                serde_json::json!({"type": "focus_lost", "platform": "windows"});
                            if event_sender
                                .send(DriverEvent::Custom("focus".to_string(), custom_event))
                                .is_err()
                            {
                        ...
                        Ok(Event::Paste(text)) => {
                            let custom_event = serde_json::json!({"type": "paste", "text": text, "platform": "windows"});
                            if event_sender
