...
impl PluginManager {
    ...
    pub fn register<P: Plugin + 'static>(&self, mut plugin: P) -> Result<()> {
        ...
        self.plugins.write().unwrap().insert(
            plugin_id.clone(),
            Arc::new(RwLock::new(boxed_plugin))
        );
        ...
        if metadata.capabilities.contains(&PluginCapability::WidgetProvider) {
            ...
            eprintln!("[Plugin] Plugin '{}' has WidgetProvider capability. Use register_widget() to register widget types.", plugin_id);
        }
        
        // Check for event interceptor capability
        if metadata.capabilities.contains(&PluginCapability::EventInterceptor) {
            self.event_interceptors.write().unwrap().push(plugin_id.clone());
        }
    ...
    pub fn unregister(&self, plugin_id: &str) -> Result<()> {
        ...
        self.load_order.write().unwrap().retain(|id| id != plugin_id);
        ...
        self.event_interceptors.write().unwrap().retain(|id| id != plugin_id);
    ...
    pub fn list_plugins(&self) -> Vec<PluginMetadata> {
        ...
        plugins.values().filter_map(|plugin_arc| {
            plugin_arc.read().ok().map(|p| p.metadata())
        }).collect()
    }
    ...
    pub fn create_widget(&self, widget_type: &str, config: WidgetConfig) -> Result<Box<dyn Component>> {
        ...
        let widget_plugin = registry.get(widget_type)
    ...
    pub fn register_widget<W: WidgetPlugin + 'static>(&self, widget: W) -> Result<()> {
        ...
        self.widget_registry.write().unwrap().insert(
            widget_type,
            Arc::new(widget)
        );
        Ok(())
    ...
    pub fn load_plugin_directory(&self, path: &str) -> Result<()> {
        ...
        if !dir_path.exists() {
            return Err(TuiError::plugin(format!("Plugin directory '{}' does not exist", path)));
        }
        
        if !dir_path.is_dir() {
            return Err(TuiError::plugin(format!("Path '{}' is not a directory", path)));
        }
        
        eprintln!("[Plugin] Scanning directory '{}' for plugins", path);
                        ...
                        match std::fs::read_to_string(&file_path) {
                            Ok(manifest_content) => {
                                match serde_json::from_str::<PluginManifest>(&manifest_content) {
                                    Ok(manifest) => {
                                        eprintln!("[Plugin] Parsed manifest for plugin: {} v{}", 
                                        ...
                                        if manifest.metadata.entry_point.ends_with(".wasm") {
                                            ...
                                            eprintln!("[Plugin] Looking for WASM module at: {}", plugin_path.display());
                                            ...
                                            if plugin_path.exists() {
                                                ...
                                                plugins.insert(manifest.metadata.id.clone(), 
                                            ...
                                            eprintln!("[Plugin] Non-WASM plugins not supported yet: {}", 
                                                      manifest.metadata.entry_point);
                                        }
                                    }
                                    Err(e) => {
                                        eprintln!("[Plugin] Failed to parse manifest {}: {}", file_path.display(), e);
                                    }
                                }
                            }
                            Err(e) => {
                                eprintln!("[Plugin] Failed to read manifest {}: {}", file_path.display(), e);
                            }
                        }
                    }
                }
        ...
        eprintln!("[Plugin] Found {} plugin manifest(s) in directory", plugin_count);
    ...
    pub fn load_config(&self, path: &str) -> Result<()> {
        ...
        eprintln!("[Plugin] Loading configuration with {} plugins from {}", config.plugins.len(), path);
        
        // Load plugins from configuration
        // We support WebAssembly modules for safe plugin execution
        for metadata in config.plugins {
            eprintln!("[Plugin] Attempting to load plugin from config: {} v{}", metadata.name, metadata.version);
...
mod example {
    use super::*;
    use crate::components::{Element, div, text};
    ...
    impl Component for CustomButtonPlugin {
        fn render(&self) -> Element {
            let text_content = self.config.properties.get("text")
                .and_then(|v| v.as_str())
                .unwrap_or("Button");
