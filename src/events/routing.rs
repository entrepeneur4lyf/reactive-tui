...
pub type ComponentEventHandler = Box<dyn Fn(&mut EventContext, &dyn Message) -> Result<()> + Send + Sync>;
...
impl EventRouter {
    ...
    pub async fn route_mouse_event(&self, mouse_event: MouseEvent) -> Result<()> {
        ...
        let target_element = {
            ...
            if let Some(ref element_id) = hit_result {
                println!("MOUSE HIT: {} at ({}, {})", element_id, mouse_event.column, mouse_event.row);
                if let Some(target) = targeting.get_component_bounds(element_id) {
                    println!("  Target bounds: {:?}", target.bounds);
                }
            } else {
                println!("MOUSE MISS: ({}, {}) - no interactive component", mouse_event.column, mouse_event.row);
            }
    ...
    pub async fn route_key_event(&self, key_event: KeyEvent) -> Result<()> {
        ...
        let target_element = {
            ...
            let focused = focus_manager.get_focused_element().map(|elem| elem.id.clone());
            
            // Debug output to show it's working
            if let Some(ref element_id) = focused {
                println!("KEY EVENT: {} -> {}", 
                    self.key_code_to_string(&key_event.code), element_id);
            } else {
                println!("KEY EVENT: {} -> no focused element", 
    ...
    fn mouse_event_to_message(&self, mouse_event: MouseEvent) -> Result<ClickMessage> {
        use crate::compat::{MouseEventKind, MouseButton};
        ...
        match mouse_event.kind {
            ...
            _ => Err(TuiError::component("Unsupported mouse event type".to_string())),
        }
    }

    /// Convert key event to appropriate message
    fn key_event_to_message(&self, key_event: KeyEvent, element_id: Option<String>) -> Result<KeyPressMessage> {
...
mod tests {
    ...
    async fn test_component_handler_registration() {
        ...
        router.register_component_handler(
