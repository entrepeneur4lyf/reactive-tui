...
impl MessageManager {
    ...
    {
        let type_id = TypeId::of::<T>();
        let mut element_handlers = self.element_handlers.write().map_err(|_| {
            TuiError::component("Failed to acquire element handlers lock".to_string())
        })?;
    ...
    pub fn process_message(&self, mut event: MessageEvent) -> Result<()> {
        ...
        while let Some(element_id) = current_element {
            ...
            current_element = {
                let hierarchy = self.hierarchy.read().map_err(|_| {
                    TuiError::component("Failed to acquire hierarchy lock".to_string())
                })?;
    ...
    fn handle_element_message(
        ...
        let element_handlers = self.element_handlers.read().map_err(|_| {
            TuiError::component("Failed to acquire element handlers lock".to_string())
        })?;
    ...
    pub fn send_async(&self, message: impl Message, sender_id: Option<String>) -> Result<()> {
        ...
        self.message_sender.send(event).map_err(|_| {
            TuiError::component("Failed to send message to async channel".to_string())
        })?;
        Ok(())
    }
    ...
    pub fn process_queue(&self) -> Result<()> {
        let messages = {
            let mut queue = self.message_queue.write().map_err(|_| {
                TuiError::component("Failed to acquire message queue lock".to_string())
            })?;
