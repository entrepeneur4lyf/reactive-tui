...
impl GridDebugger {
    ...
    fn analyze_item_placements(&self, element: &Element, layout: &Layout) -> Vec<GridItemReport> {
            ...
            .enumerate()
        {
            ...
            let placement_type =
                if item_config.column_start.is_some() || item_config.row_start.is_some() {
                    PlacementType::Explicit
    ...
    fn calculate_statistics(
        ...
        let efficiency_score =
            self.calculate_efficiency_score(occupancy_rate, items, container_rect);
    ...
    fn detect_warnings(
        ...
        if stats.efficiency_score < 60.0 {
            warnings.push(GridWarning {
                ...
                message: format!("Grid efficiency is low ({:.1}/100). Consider reducing gaps or adjusting item sizes.", stats.efficiency_score),
                item_index: None,
                suggestion: Some("Try using smaller gaps or better-sized items for the content.".to_string()),
            });
        }
        ...
        for (index, item) in items.iter().enumerate() {
            ...
            if item.content.is_none() && item.span.0 * item.span.1 > 1 {
                warnings.push(GridWarning {
                    ...
                    item_index: Some(index),
                    suggestion: Some(
                        "Consider reducing span or adding content to justify the space."
                            .to_string(),
                    ),
                });
            }
        ...
        if items.len() > 50 {
            warnings.push(GridWarning {
                ...
                item_index: None,
                suggestion: Some(
                    "Consider pagination, virtualization, or grouping items.".to_string(),
                ),
            });
        }
    ...
    fn render_grid_lines(&self, _layout: &Layout, container_rect: LayoutRect) -> String {
        ...
        for row in 0..container_rect.height.saturating_sub(2) {
            ...
            if self.config.show_coordinates && row == 0 {
                let coord_text =
                    format!(" Grid {}×{} ", container_rect.width, container_rect.height);
