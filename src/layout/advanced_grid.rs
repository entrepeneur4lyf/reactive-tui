...
impl GridLayout {
  ...
  fn build_grid_placements(
    ...
    for (index, child) in children.iter().enumerate() {
      ...
      if let (Some(col_start), Some(row_start)) = (item_config.column_start, item_config.row_start)
      {
    ...
    for (index, child, item_config) in auto_items {
      ...
      let span_cols = (item_config.column_span as usize).min(column_count.saturating_sub(col));
  ...
  #[allow(dead_code)]
  fn calculate_cell_rect(&self, params: CellRectParams, container_rect: LayoutRect) -> LayoutRect {
