...
impl GridLayout {
    ...
    pub fn compute_layout(
        ...
        let rows =
            self.resolve_tracks(&self.config.rows, row_count, container_rect.height, false)?;
        ...
        for (element_id, cell) in cell_map.iter() {
            ...
            let placement =
                self.create_placement(element, *cell, &columns, &rows, container_rect)?;
    ...
    fn build_cell_map(
        ...
        let column_count = self.config.column_count.unwrap_or_else(|| {
            if let Some(min_width) = self.config.min_column_width {
                ...
                let cols = (available_width + self.config.column_gap)
                    / (min_width + self.config.column_gap);
    ...
    fn create_placement(
        ...
        let start_row = rows.get(cell.row).ok_or_else(|| {
            crate::error::TuiError::LayoutError("Row index out of bounds".to_string())
        })?;
