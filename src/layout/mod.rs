...
impl LayoutEngine {
    ...
    pub fn compute_layout_with_styles(&mut self, element: &Element, styles: &ComputedStyles) -> Result<Layout> {
        let available_rect = LayoutRect {
            x: 0,
            y: 0,
            width: self.terminal_width,
    ...
    fn compute_flex_children(
        ...
        let is_row = matches!(styles.flex_direction, FlexDirection::Row | FlexDirection::RowReverse);
        let available_space = if is_row { container_rect.width } else { container_rect.height };
        ...
        let fr_unit = if total_fr > 0.0 { remaining_space as f32 / total_fr } else { 0.0 };
        
        // Second pass: compute actual layouts
        let mut current_pos = if is_row { container_rect.x } else { container_rect.y };
        ...
        for (i, child) in element.children.iter().enumerate() {
            ...
            let computed_size = match size {
                ...
                SizeValue::Auto => {
                    if total_fr == 0.0 && element.children.len() > 0 {
            ...
            current_pos += if is_row { child_layout.rect.width } else { child_layout.rect.height };
    ...
    fn compute_block_children(
        ...
        for child in &element.children {
            ...
            let child_layout = self.compute_layout_recursive(child, child_rect, &child_styles)?;
            current_y +=
                child_layout.rect.height + child_styles.margin.top + child_styles.margin.bottom;
