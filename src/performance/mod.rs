...
impl PerformanceMonitor {
  ...
  pub fn record_event_processing(&mut self, event_type: &str, processing_time: Duration) {
    ...
    metrics
      .event_processing_avg
      .insert(event_type.to_string(), avg_time);
  }
  ...
  pub fn record_css_processing(&mut self, processing_time: Duration) {
    ...
    if !self.css_processing_times.is_empty() {
      metrics.css_processing_avg =
        self.css_processing_times.iter().sum::<Duration>() / self.css_processing_times.len() as u32;
    }
  }
  ...
  pub fn record_layout_computation(&mut self, computation_time: Duration) {
    ...
    if !self.layout_computation_times.is_empty() {
      metrics.layout_computation_avg = self.layout_computation_times.iter().sum::<Duration>()
        / self.layout_computation_times.len() as u32;
    }
  }
  ...
  fn calculate_performance_score(&self, metrics: &PerformanceMetrics) -> f64 {
    ...
    let memory_score = if metrics.memory_growth_rate <= 0.0 {
      100.0
    } else if metrics.memory_growth_rate < 1024.0 {
      // < 1KB/s growth
      80.0
    } else if metrics.memory_growth_rate < 10240.0 {
      // < 10KB/s growth
      60.0
    } else {
      40.0
  ...
  fn identify_bottlenecks(&self, metrics: &PerformanceMetrics) -> Vec<PerformanceBottleneck> {
    ...
    if metrics.current_fps < metrics.target_fps * 0.8 {
      bottlenecks.push(PerformanceBottleneck {
        ...
        description: format!(
          "Low frame rate: {:.1} FPS (target: {:.1})",
          metrics.current_fps, metrics.target_fps
        ),
        impact_score: (metrics.target_fps - metrics.current_fps) / metrics.target_fps * 10.0,
        suggested_fix: "Consider reducing visual complexity or optimizing render pipeline"
          .to_string(),
      });
    }

    // Check memory growth
    if metrics.memory_growth_rate > 10240.0 {
      // > 10KB/s
      bottlenecks.push(PerformanceBottleneck {
        category: BottleneckCategory::Memory,
        severity: if metrics.memory_growth_rate > 102400.0 {
          // > 100KB/s
          BottleneckSeverity::Critical
        } else {
          BottleneckSeverity::High
        ...
        description: format!(
          "Memory leak detected: {:.1} KB/s growth",
          metrics.memory_growth_rate / 1024.0
        ),
    ...
    if metrics.render_efficiency > 0.8 {
      bottlenecks.push(PerformanceBottleneck {
        ...
        description: format!(
          "High render time: {:.1}% of frame time",
          metrics.render_efficiency * 100.0
        ),
    ...
    if metrics.css_processing_avg > Duration::from_millis(2) {
      bottlenecks.push(PerformanceBottleneck {
        ...
        severity: BottleneckSeverity::Medium,
        description: format!(
          "Slow CSS processing: {:.1}ms",
          metrics.css_processing_avg.as_secs_f64() * 1000.0
        ),
    ...
    if metrics.layout_computation_avg > Duration::from_millis(3) {
      bottlenecks.push(PerformanceBottleneck {
        ...
        severity: BottleneckSeverity::Medium,
        description: format!(
          "Slow layout computation: {:.1}ms",
          metrics.layout_computation_avg.as_secs_f64() * 1000.0
        ),
  ...
  fn generate_recommendations(&self, metrics: &PerformanceMetrics) -> Vec<String> {
    ...
    if metrics.current_fps < metrics.target_fps * 0.9 {
      recommendations
        .push("Consider reducing target FPS or optimizing render pipeline".to_string());
    }
    ...
    if metrics.reactive_updates_per_second > 30.0 {
      recommendations
        .push("High reactive update frequency - consider batching updates".to_string());
    }

    if metrics.render_efficiency > 0.7 {
      recommendations.push(
        "Rendering takes significant portion of frame time - optimize render operations"
          .to_string(),
      );
    }
