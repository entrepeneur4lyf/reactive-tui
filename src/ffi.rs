...
// Removed unused import: use napi::bindgen_prelude::*;
...
impl JsTuiApp {
    ...
    pub fn set_component(&self, element: &JsElement) -> napi::Result<()> {
        ...
        app.set_component(Box::new(component))
    ...
    pub fn start(&self) -> napi::Result<String> {
        ...
        if !app.has_component() {
            return Err(napi::Error::from_reason("No component set. Call set_component() first."));
        }
    ...
    pub fn get_state_json(&self) -> napi::Result<String> {
        ...
        for (key, value) in fields.iter() {
            ...
            if let Some(string_val) = value.downcast_ref::<String>() {
                ...
                json_map.insert(key.clone(), serde_json::Value::Number(serde_json::Number::from(*i32_val)));
            } else if let Some(i64_val) = value.downcast_ref::<i64>() {
                json_map.insert(key.clone(), serde_json::Value::Number(serde_json::Number::from(*i64_val)));
            } else if let Some(u32_val) = value.downcast_ref::<u32>() {
                json_map.insert(key.clone(), serde_json::Value::Number(serde_json::Number::from(*u32_val)));
            } else if let Some(u64_val) = value.downcast_ref::<u64>() {
                json_map.insert(key.clone(), serde_json::Value::Number(serde_json::Number::from(*u64_val)));
                ...
                let arr = vec_str.iter().map(|s| serde_json::Value::String(s.clone())).collect();
                json_map.insert(key.clone(), serde_json::Value::Array(arr));
            } else {
                // For unknown types, store as a type indicator string
                json_map.insert(key.clone(), serde_json::Value::String(format!("<{}: unsupported type>", key)));
            }
        }
    ...
    pub fn set_state_json(&self, json: String) -> napi::Result<()> {
        ...
        if let serde_json::Value::Object(map) = value {
            ...
            for (key, val) in map {
                ...
                let _ = self.state.change_sender().send(crate::reactive::StateChange {
                    field_name: key,
