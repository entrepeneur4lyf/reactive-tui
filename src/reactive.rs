...
/// # Example
/// ```rust
/// use tui_core::reactive::{Reactive, ReactiveState};
/// use tui_core::impl_reactive_struct;
/// 
/// struct MyComponent {
    ...
    fn init_reactive(&mut self) {
        ...
        tokio::spawn(async move {
            while let Ok(change) = state_changes.recv().await {
                eprintln!("[Reactive] Field '{}' changed at {:?}", 
                          change.field_name, 
                          change.timestamp);
            }
        });
...
mod tests {
    ...
    fn test_reactive_struct_trait() {
        ...
        assert_eq!(state.get_field::<String>("numbers"), Some("Count: 1".to_string()));
