    ...
    pub async fn schedule_update(&mut self, request: UpdateRequest) -> Result<()> {
        ...
        if let Some(existing) = self.pending_components.get(&component_id).cloned() {
            ...
            if request.priority > existing.priority {
                self.pending_components.insert(component_id.clone(), request.clone());
        ...
        // Add new request
        self.pending_components.insert(component_id, request.clone());
        self.update_queues
            .entry(priority)
            .or_default()
            .push_back(request);
    ...
    pub async fn get_pending_updates(&mut self) -> Result<Vec<UpdateRequest>> {
        ...
        if !batch.is_empty() {
            ...
            self.stats.average_batch_size =
                (self.stats.average_batch_size * (self.stats.total_updates_batched - 1) as f64 + batch.len() as f64)
