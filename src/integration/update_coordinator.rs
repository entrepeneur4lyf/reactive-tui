...
impl UpdateCoordinator {
    ...
    pub fn unbind_reactive_from_component(&mut self, reactive_id: &ReactiveId, component_id: &ComponentId) {
        ...
        // Remove binding
        self.bindings.remove(&(reactive_id.clone(), component_id.clone()));
    }
    ...
    pub fn get_binding(&self, reactive_id: &ReactiveId, component_id: &ComponentId) -> Option<&ReactiveBinding> {
        self.bindings.get(&(reactive_id.clone(), component_id.clone()))
    }
        ...
        BindingStats {
            ...
            average_components_per_reactive: if self.reactive_to_components.is_empty() {
                ...
                self.reactive_to_components
                    ...
                    .sum::<usize>() as f64 / self.reactive_to_components.len() as f64
            },
        }
    }
