...
mod tests {
  ...
  use crate::{
    ...
    error::Result,
    integration::{
      ComponentId, ComponentInstance, ComponentInstanceManager, ReactiveBinding,
      ReactiveChangeEvent, ReactiveIntegration, ReactiveUpdateScheduler, UpdateCoordinator,
      UpdateRequest,
    },
    reactive::Reactive,
  };
  ...
  impl Component for TestReactiveComponent {
    ...
    fn on_mount(&mut self, context: &mut crate::components::ComponentContext) -> Result<()> {
      println!(
        "TestReactiveComponent mounted with ID: {}",
    ...
    fn on_unmount(&mut self, context: &mut crate::components::ComponentContext) -> Result<()> {
      println!(
        "TestReactiveComponent unmounted with ID: {}",
        context.component_id
      );
      Ok(())
    }
  ...
  async fn test_reactive_integration_multiple_components() -> Result<()> {
    ...
    let bindings = vec![ReactiveBinding::new(
