...
impl ImageWidget {
            ...
            ImageSource::EmbeddedData(data, format_hint) => {
                let img = if let Some(format_str) = format_hint {
                    ...
                    let format = match format_str.as_str() {
                        ...
                        "bmp" => ImageFormat::Bmp,
                        _ => return Err(TuiError::component(format!("Unsupported image format: {format_str}"))),
                    };
    ...
    #[cfg(feature = "images")]
    fn scale_image_for_bounds(&self, image: &DynamicImage, bounds: LayoutRect) -> Result<DynamicImage> {
        ...
        match self.config.scaling {
            ...
            ScalingMode::Fill => {
                Ok(image.resize_to_fill(target_width, target_height, image::imageops::FilterType::Lanczos3))
            }
            ScalingMode::Stretch => {
...
mod tests {
    ...
    fn test_image_fallback_placeholder() {
        ...
        let bounds = LayoutRect { x: 0, y: 0, width: 20, height: 10 };
