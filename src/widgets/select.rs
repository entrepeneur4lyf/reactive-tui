...
//!
//! ```rust
//! use tui_core::widgets::{Select, SelectBuilder};
//!
            ...
            match state.highlighted_index {
                ...
                Some(current) => {
                    if let Some(current_pos) =
                        state.filtered_indices.iter().position(|&i| i == current)
                    {
            ...
            match state.highlighted_index {
                None => {
                    state.highlighted_index =
                        Some(state.filtered_indices[state.filtered_indices.len() - 1]);
                }
                Some(current) => {
                    if let Some(current_pos) =
                        state.filtered_indices.iter().position(|&i| i == current)
    ...
    fn create_dropdown_element(&self, state: &SelectState) -> Element {
        ...
        for &option_index in visible_options {
            ...
            if state.highlighted_index == Some(option_index) {
                option_element =
                    option_element.classes(self.style.highlighted_option_classes.clone());
            }
