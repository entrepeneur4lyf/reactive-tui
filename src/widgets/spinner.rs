...
impl Spinner {
  ...
  pub fn update(&mut self) -> Result<bool> {
    ...
    if let Some(last_update) = self.state.last_update {
      ...
      if elapsed >= interval {
        self.state.current_frame = (self.state.current_frame + 1) % self.definition.frames.len();
  ...
  fn get_predefined_definition(spinner_type: SpinnerType) -> SpinnerDefinition {
    match spinner_type {
      ...
      SpinnerType::Dots2 => {
        SpinnerDefinition::from_static(&["⣾", "⣽", "⣻", "⢿", "⡿", "⣟", "⣯", "⣷"], 80, Some("dots2"))
      }
      SpinnerType::Line => {
        SpinnerDefinition::from_static(&["-", "\\", "|", "/"], 130, Some("line"))
      }
      SpinnerType::Pipe => {
        SpinnerDefinition::from_static(&["┤", "┘", "┴", "└", "├", "┌", "┬", "┐"], 100, Some("pipe"))
      }
      SpinnerType::SimpleDots => {
        SpinnerDefinition::from_static(&[".  ", ".. ", "...", "   "], 400, Some("simpleDots"))
      }
      ...
      SpinnerType::Star2 => SpinnerDefinition::from_static(&["+", "x", "*"], 80, Some("star2")),
      SpinnerType::Arc => {
        SpinnerDefinition::from_static(&["◜", "◠", "◝", "◞", "◡", "◟"], 100, Some("arc"))
      }
      SpinnerType::Circle => SpinnerDefinition::from_static(&["◡", "⊙", "◠"], 120, Some("circle")),
      ...
      SpinnerType::Toggle2 => SpinnerDefinition::from_static(&["▫", "▪"], 80, Some("toggle2")),
      SpinnerType::Toggle3 => SpinnerDefinition::from_static(&["□", "■"], 120, Some("toggle3")),
      ...
      SpinnerType::Balloon => {
        SpinnerDefinition::from_static(&[" ", ".", "o", "O", "@", "*", " "], 140, Some("balloon"))
      }
      SpinnerType::Noise => SpinnerDefinition::from_static(&["▓", "▒", "░"], 100, Some("noise")),
      ...
      SpinnerType::Hearts => {
        SpinnerDefinition::from_static(&["💛 ", "💙 ", "💜 ", "💚 ", "❤️ "], 100, Some("hearts"))
      }
      ...
      SpinnerType::Smiley => SpinnerDefinition::from_static(&["😄 ", "😝 "], 200, Some("smiley")),
      SpinnerType::Monkey => {
        SpinnerDefinition::from_static(&["🙈 ", "🙈 ", "🙉 ", "🙊 "], 300, Some("monkey"))
      }
      SpinnerType::Runner => SpinnerDefinition::from_static(&["🚶 ", "🏃 "], 140, Some("runner")),
