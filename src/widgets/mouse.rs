...
impl MouseWidget {
  ...
  pub fn handle_mouse_event(&mut self, event: MouseEvent) -> Result<bool> {
    let bounds = self
      .bounds
      .ok_or_else(|| TuiError::render("Mouse widget bounds not set".to_string()))?;
  ...
  fn handle_mouse_drag(&mut self, button: MouseButton, position: MousePosition) -> Result<()> {
    ...
    if let Some(ref mut drag_state) = self.drag_state {
      if drag_state.button == button_type {
        let distance =
          Self::calculate_distance_static(drag_state.start_position, (position.x, position.y));
  ...
  fn handle_hover_enter(&mut self, position: MousePosition) -> Result<()> {
    ...
    if let Some(callback) = &self.config.on_hover_enter {
      let action = self
        ...
        .param(
          "relative_position",
          format!("({}, {})", position.relative_x, position.relative_y),
        )
        .build();
      self.action_dispatcher.dispatch(action);
    }
    ...
    if let Some(callback) = &self.config.on_drag_end {
      ...
      let action = self
        ...
        .param(
          ...
          format!(
            "({}, {})",
            drag_state.start_position.0, drag_state.start_position.1
          ),
        )
...
impl ResponsiveWidget for MouseWidget {
  fn to_element(&self) -> Element {
    let mut builder = Element::with_tag("div")
      ...
      .attr(
        "data-cursor",
        format!("{:?}", self.config.cursor_style).to_lowercase(),
      )
      ...
      .attr(
        ...
        self.config.enable_gestures.to_string(),
      );
...
pub fn click_area(id: &str, on_click: &str) -> Element {
  mouse(id, |config| {
    config.on_click(on_click).cursor_style(CursorStyle::Pointer)
  })
}
...
mod tests {
  ...
  fn test_drag_drop_factory_functions() {
    ...
    assert_eq!(
      draggable_elem.attributes.get("data-cursor"),
      Some(&"grab".to_string())
    );
    ...
    assert_eq!(
      droppable_elem.attributes.get("data-cursor"),
    ...
    assert!(drop_target_elem
      .classes
      .contains(&"drop-target".to_string()));
    assert_eq!(
      drop_target_elem.attributes.get("data-cursor"),
      Some(&"default".to_string())
    );
    assert_eq!(
      drop_target_elem.attributes.get("data-enable-drag"),
      Some(&"true".to_string())
    );
  }
}
