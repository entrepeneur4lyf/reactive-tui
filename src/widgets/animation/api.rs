...
fn convert_color_value_to_animated(color: &ColorValue) -> AnimatedProperty {
  match color {
    ...
    ColorValue::FromTo { from, to } => {
      let from_color = ColorDefinition {
        r: from.0,
        g: from.1,
        b: from.2,
      };
...
fn convert_position_value_to_animated(position: &PositionValue) -> AnimatedProperty {
  match position {
    ...
    PositionValue::FromTo { from, to } => AnimatedProperty::Position(from.0, from.1, to.0, to.1),
  }
}
...
mod tests {
  ...
  fn test_property_value_conversion() {
    let opacity_prop =
      convert_property_value_to_animated("opacity", &PropertyValue::FromTo { from: 0.0, to: 1.0 });
