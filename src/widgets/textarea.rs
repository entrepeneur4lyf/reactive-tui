...
impl Textarea {
    ...
    fn fix_scroll(&mut self) {
        ...
        {
            self.state.viewport.scroll_row =
                self.state.cursor.row - self.state.viewport.visible_rows + 1;
        }
        ...
        {
            self.state.viewport.scroll_col =
                self.state.cursor.col - self.state.viewport.visible_cols + 1;
        }
    }
    ...
    fn highlight_keywords(&self, line: &str, keywords: &[&str]) -> String {
        ...
        let highlight_color = ColorDefinition { r: 0, g: 255, b: 255 };
        ...
        for keyword in keywords {
            if result.contains(keyword) {
                ...
                result = result.replace(
                    keyword, 
                    &format!("{}{}{}", highlight_ansi, keyword, reset_ansi)
                );
            }
        }
...
impl EditOperation {
    ...
    pub fn inverse(&self) -> EditOperation {
        match self {
            EditOperation::InsertChar { pos, ch } => {
                EditOperation::DeleteChar { pos: *pos, ch: *ch }
            }
            EditOperation::DeleteChar { pos, ch } => {
                EditOperation::InsertChar { pos: *pos, ch: *ch }
            }
