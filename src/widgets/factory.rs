...
/// Type alias for complex widget instance storage
type WidgetInstanceMap = HashMap<String, Arc<Mutex<Box<dyn WidgetInstance>>>>;
...
pub struct WidgetRegistry {
  ...
  instances: Arc<Mutex<WidgetInstanceMap>>,
  #[allow(dead_code)]
...
impl WidgetRegistry {
  ...
  pub fn register<C: WidgetConfig + 'static>(
    ...
    if builders.contains_key(&widget_type) {
      return Err(crate::error::TuiError::component(format!(
        "Widget type already exists: {widget_type}"
      )));
    }
  ...
  pub fn create_widget<C: WidgetConfig + 'static>(
    ...
    let builder = builder_any
      .downcast_ref::<WidgetBuilder<C>>()
      .ok_or_else(|| {
        WidgetFactoryError::CreationFailed(format!(
          "Widget builder type mismatch for: {widget_type}"
        ))
      })?;
...
pub static WIDGET_FACTORY: Lazy<Arc<WidgetRegistry>> =
  Lazy::new(|| Arc::new(WidgetRegistry::new()));
...
macro_rules! register_widget_type {
  ($widget_type:literal, $config_type:ty, $widget_class:ty) => {{
    ...
    let builder: WidgetBuilder<$config_type> =
      Box::new(|config| Box::new(<$widget_class>::new(config)));
...
pub mod base {
  ...
  impl BaseWidget {
    ...
    pub fn create_base_element(&self) -> crate::components::Element {
      ...
      let mut element = div().id(&self.config.id);
  ...
  impl WidgetInstance for BaseWidget {
    ...
    fn render(&mut self) -> Result<Element> {
      if self.destroyed {
        return Err(crate::error::TuiError::component(format!(
          "Cannot render destroyed widget: {}",
          self.id()
        )));
      }
    ...
    fn update(&mut self, _updates: Box<dyn std::any::Any + Send + Sync>) -> Result<()> {
      if self.destroyed {
        return Err(crate::error::TuiError::component(format!(
          "Cannot update destroyed widget: {}",
          self.id()
        )));
      }
    ...
    fn validate(&self) -> bool {
      !self.destroyed && !self.config.id.is_empty() && !self.config.widget_type.is_empty()
    }
