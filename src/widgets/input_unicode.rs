...
pub fn prev_grapheme_boundary(s: &str, byte_idx: usize) -> usize {
  ...
  let mut last = 0usize;
  for (i, _) in s[..byte_idx].grapheme_indices(true) { last = i; }
  last
}

pub fn next_grapheme_boundary(s: &str, byte_idx: usize) -> usize {
  if byte_idx >= s.len() { return s.len(); }
  for (i, g) in s[byte_idx..].grapheme_indices(true) { return byte_idx + i + g.len(); }
  s.len()
}

pub fn byte_index_for_display_col(s: &str, cols: usize) -> usize {
  if cols == 0 { return 0; }
  let mut acc = 0usize; let mut idx = 0usize; let mut i = 0usize;
  while i < s.len() {
    if let Some(end) = ansi_token_end(s, i) { i = end; idx = i; continue; }
    if let Some((rel, g)) = s[i..].grapheme_indices(true).next() {
      let w = UnicodeWidthStr::width(g);
      if acc + w > cols { break; }
      acc += w; i = i + rel + g.len(); idx = i;
    } else { break; }
  }
  idx
}

pub fn visible_slice_by_width<'a>(s: &'a str, start_cols: usize, max_cols: usize) -> (&'a str, usize, usize) {
  let mut acc = 0usize;
  let mut end_byte;
  ...
  end_byte = start_byte;
  while i < s.len() {
    if let Some(end) = ansi_token_end(s, i) { i = end; end_byte = i; continue; }
    if let Some((rel, g)) = s[i..].grapheme_indices(true).next() {
      let w = UnicodeWidthStr::width(g);
      if acc + w > max_cols { break; }
      acc += w; i = i + rel + g.len(); end_byte = i;
    } else { break; }
  }
