...
impl LinkWidget {
    ...
    pub fn is_file_path(&self) -> bool {
        ...
        self.config.url.starts_with("../") ||
        self.config.url.starts_with('/') ||
        (cfg!(windows) && self.config.url.len() > 2 && 
         self.config.url.chars().nth(1) == Some(':'))
    }
    ...
    fn execute_command(&self) -> Result<()> {
        ...
        if self.config.target != LinkTarget::Command {
            return Err(TuiError::component("Command execution not allowed for this link"));
        }
    ...
    fn get_css_classes(&self) -> Vec<String> {
        let mut classes = vec![
            ...
            format!("link-decoration-{}", match self.config.decoration {
...
mod tests {
    ...
    fn test_url_type_detection() {
        let web_link = LinkWidget::new("web", LinkConfig {
        ...
        let email_link = LinkWidget::new("email", LinkConfig {
        ...
        let file_link = LinkWidget::new("file", LinkConfig {
    ...
    fn test_url_validation() {
        let valid_web = LinkWidget::new("web", LinkConfig {
            url: "https://example.com".to_string(),
            ..LinkConfig::default()
        });
        assert!(valid_web.validate_url().is_ok());

        let valid_email = LinkWidget::new("email", LinkConfig {
            url: "test@example.com".to_string(),
            ..LinkConfig::default()
        });
        assert!(valid_email.validate_url().is_ok());

        let invalid_empty = LinkWidget::new("empty", LinkConfig {
            url: "".to_string(),
            ..LinkConfig::default()
        });
        assert!(invalid_empty.validate_url().is_err());

        let invalid_email = LinkWidget::new("bad-email", LinkConfig {
    ...
    fn test_link_state_management() {
        ...
        let mut disabled_widget = LinkWidget::new("disabled", LinkConfig {
            disabled: true,
            ..LinkConfig::default()
        });
    ...
    fn test_display_text_with_indicator() {
        let external_link = LinkWidget::new("external", LinkConfig {
        ...
        let internal_link = LinkWidget::new("internal", LinkConfig {
    ...
    fn test_factory_functions() {
        ...
        assert_eq!(email.attributes.get("href"), Some(&"mailto:test@example.com".to_string()));

        let file = file_link("file", "./README.md", "Read Me");
        assert_eq!(file.attributes.get("href"), Some(&"./README.md".to_string()));
    }
}