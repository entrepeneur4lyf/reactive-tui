...
pub type LazyLoader =
  Arc<dyn Fn(&NodeId) -> Pin<Box<dyn Future<Output = Result<Vec<TreeNode>>> + Send>> + Send + Sync>;
    ...
    if let Some(node) = nodes.get(node_id) {
      ...
      if node.expanded && state.expanded_nodes.contains(node_id) {
        for child_id in &node.children {
          Self::collect_visible_nodes_recursive(child_id, nodes, state, visible, _level + 1);
        }
      }
    }
  }
