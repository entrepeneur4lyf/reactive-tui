...
fn textarea_grapheme_cursor_navigation() {
  ...
  ta.move_cursor_left();
  assert!(ta.state.cursor.col < col_before, "cursor should move left by grapheme");
  // Move right back
  let col_left = ta.state.cursor.col;
  ta.move_cursor_right();
  assert!(ta.state.cursor.col > col_left, "cursor should move right by grapheme");
}
...
fn rich_text_wrap_ansi_unicode_width() {
  ...
  let mut rt = RichTextBuilder::new("rt").content(content).width(8).word_wrap(true).build();
  rt.parse_markdown();
  rt.render_content();
  ...
  for line in &rt.rendered_lines {
    assert!(visible_width(line.as_str()) <= 8, "wrapped line exceeds width: {:?}", line);
  }
}
