...
// Additional tests: FFI error-path coverage

test('Error handling - start without component', t => {
    const app = new JsTuiApp()
    const err = t.throws(() => app.start())
    t.truthy(err)
    t.true(String(err?.message || err).includes('No component set'), 'Should surface helpful error from Rust')
})

test('Error handling - invalid CSS path in loadStylesheet', t => {
    const app = new JsTuiApp()
    const err = t.throws(() => app.loadStylesheet('/path/does/not/exist.css'))
    t.truthy(err)
    t.true(String(err?.message || err).includes('Failed to read stylesheet'), 'Should surface IO error reason')
})
