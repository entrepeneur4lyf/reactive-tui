...
use reactive_tui::layout::grid::{GridConfig, GridLayout, GridScalar};
...
fn main() -> Result<()> {
  ...
  let (term_width, term_height) = crossterm::terminal::size().unwrap_or((400, 200));
  ...
  let grid_layout_vertical =
    GridLayout::new(grid_config_vertical).with_dimensions(term_width, term_height);
  ...
  let grid_layout_horizontal =
    GridLayout::new(grid_config_horizontal).with_dimensions(term_width, term_height);
  ...
  let grid_layout_mixed =
    GridLayout::new(grid_config_mixed).with_dimensions(term_width, term_height);
...
fn render_grid_placements(placements: &[reactive_tui::layout::grid::GridPlacement]) {
