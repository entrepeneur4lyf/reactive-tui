...
use reactive_tui::integration::{ReactiveBinding, ReactiveChangeEvent, ComponentId};
use std::sync::Arc;
...
impl Component for CounterComponent {
    ...
    fn on_mount(&mut self, _context: &mut reactive_tui::components::ComponentContext) -> reactive_tui::error::Result<()> {
        println!("CounterComponent mounted!");
        Ok(())
    }

    fn on_unmount(&mut self, _context: &mut reactive_tui::components::ComponentContext) -> reactive_tui::error::Result<()> {
...
async fn main() -> reactive_tui::error::Result<()> {
    ...
    let component_id = app.mount_reactive_component(
        Box::new(counter_component),
        counter_bindings
    ).await?;
    ...
    tokio::spawn(async move {
        ...
        loop {
            ...
            println!("Reactive values updated: count={}, message={}", count, message_clone.get());
    ...
    app.bind_key_to_action(
        reactive_tui::events::KeyCombination::char('q'),
        "quit"
    ).await;
