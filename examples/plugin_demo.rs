...
impl Plugin for GaugeWidgetPlugin {
    ...
    fn metadata(&self) -> PluginMetadata {
        PluginMetadata {
            ...
            tags: vec!["widget".to_string(), "gauge".to_string(), "visualization".to_string()],
        }
    }
    ...
    fn handle_event(&mut self, event: &PluginEvent) -> Option<PluginResponse> {
        match event {
            PluginEvent::Custom { event_type, data: _ } if event_type == "theme-change" => {
...
impl Component for GaugeWidgetPlugin {
    fn render(&self) -> Element {
        ...
        let label = self.config.properties.get("label")
            .and_then(|v| v.as_str())
            .unwrap_or("Gauge");
        let _size = self.config.properties.get("size")
            .and_then(|v| v.as_str())
            .unwrap_or("medium");
...
fn main() -> Result<()> {
    ...
    for (i, value) in gauge_values.iter().enumerate() {
        ...
        match plugin_manager.read().unwrap().create_widget("gauge", config) {
            Ok(widget) => {
    ...
    plugin_manager.read().unwrap().broadcast_event(PluginEvent::Custom {
