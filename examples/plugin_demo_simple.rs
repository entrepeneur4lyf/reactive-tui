...
impl Plugin for SimpleWidgetPlugin {
  ...
  fn initialize(&mut self, context: &mut PluginContext) -> reactive_tui::error::Result<()> {
    println!("🔌 Simple Widget Plugin initialized");
    context.set_state("initialized", "true");
    Ok(())
  }

  fn cleanup(&mut self) -> reactive_tui::error::Result<()> {
...
impl Plugin for EventLoggerPlugin {
  ...
  fn initialize(&mut self, _context: &mut PluginContext) -> reactive_tui::error::Result<()> {
