...
impl FlickeringTest {
  ...
  fn new() -> reactive_tui::error::Result<Self> {
  ...
  async fn render(&mut self) -> reactive_tui::error::Result<()> {
  ...
  async fn render_panels(&mut self) -> reactive_tui::error::Result<()> {
...
fn main() -> reactive_tui::error::Result<()> {
  ...
  let runtime = tokio::runtime::Runtime::new().map_err(|e| {
    reactive_tui::error::TuiError::driver(format!("Failed to create async runtime: {e}"))
  })?;
...
mod tests {
  ...
  fn test_frame_buffer_functionality() {
    use reactive_tui::rendering::FrameBuffer;
