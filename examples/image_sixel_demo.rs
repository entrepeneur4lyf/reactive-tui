...
impl Component for ImageDemo {
  fn render(&self) -> Element {
    ...
    Element::with_tag("div")
      .class("image-demo")
      .child(Element::with_tag("h1").content("Sixel Image Demo").build())
      .child(image("demo-image", |config| {
        config
      ...
      .child(Element::with_tag("p").content("Press ESC to quit").build())
      .build()
  }
}
...
async fn main() -> reactive_tui::error::Result<()> {
  ...
  app
    .bind_key_to_action(reactive_tui::events::KeyCombination::escape(), "quit")
    .await;
