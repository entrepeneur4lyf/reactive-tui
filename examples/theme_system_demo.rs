...
impl ThemeManager {
  fn new() -> Self {
    ...
    let dark_theme = ThemeDefinition {
      ...
      .collect(),
      styles: [("border", "rounded"), ("shadow", "md"), ("opacity", "90")]
        .iter()
        .map(|(k, v)| (k.to_string(), v.to_string()))
        .collect(),
    };
    ...
    let light_theme = ThemeDefinition {
      ...
      .map(|(k, v)| (k.to_string(), v.to_string()))
      .collect(),
      styles: [("border", "solid"), ("shadow", "lg"), ("opacity", "95")]
        .iter()
        .map(|(k, v)| (k.to_string(), v.to_string()))
        .collect(),
    };
    ...
    let custom_theme = ThemeDefinition {
      ...
      .map(|(k, v)| (k.to_string(), v.to_string()))
      .collect(),
      styles: [("border", "double"), ("shadow", "xl"), ("opacity", "85")]
        .iter()
        .map(|(k, v)| (k.to_string(), v.to_string()))
        .collect(),
    };
  ...
  fn reload_themes(&mut self) -> Result<()> {
    ...
    self
      .themes
      .retain(|k, _| builtin_keys.contains(&k.as_str()));
  ...
  div()
    ...
    .children(
      theme
        .colors
        .iter()
        .map(|(name, color)| create_color_preview(name, color))
        .collect::<Vec<_>>(),
    )
    .child(text("").build())
...
fn main() -> Result<()> {
...

  {
    ...
    println!(
      "Average colors per theme: {:.1}",
      total_colors as f64 / manager.themes.len() as f64
    );
    println!(
      "Average styles per theme: {:.1}",
      total_styles as f64 / manager.themes.len() as f64
    );
  }
