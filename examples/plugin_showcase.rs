...
impl Plugin for GaugeWidget {
  ...
  fn handle_event(&mut self, _: &PluginEvent) -> Option<PluginResponse> {
    None
  }
  fn as_any(&self) -> &dyn std::any::Any {
    self
...
impl WidgetPlugin for GaugeWidget {
  ...
  fn validate_config(&self, _: &WidgetConfig) -> Result<()> {
    Ok(())
  }
}
  ...
  fn new() -> Self {
    ...
    themes.insert(
      ...
      ThemeDefinition {
        ...
        .collect(),
        styles: [("border", "neon"), ("animation", "glitch")]
          .iter()
          .map(|(k, v)| (k.to_string(), v.to_string()))
          .collect(),
      },
    ...
    themes.insert(
      ...
      ThemeDefinition {
        ...
        .map(|(k, v)| (k.to_string(), v.to_string()))
        .collect(),
        styles: [("border", "organic"), ("animation", "smooth")]
          .iter()
          .map(|(k, v)| (k.to_string(), v.to_string()))
          .collect(),
      },
...
impl Plugin for ThemeProviderPlugin {
  ...
  fn initialize(&mut self, context: &mut PluginContext) -> Result<()> {
    println!(
      "🎨 Theme Provider v1.5.0 loaded with {} themes",
...
impl Plugin for DataSourcePlugin {
  ...
  fn initialize(&mut self, context: &mut PluginContext) -> Result<()> {
    println!(
      "📡 Data Source v3.0.0 initializing {} data streams",
  ...
  fn handle_event(&mut self, event: &PluginEvent) -> Option<PluginResponse> {
    match event {
      PluginEvent::Custom {
        event_type,
        data: _,
      } if event_type == "get-streams" => {
        let streams = self.data_streams.read().unwrap();
        let stream_data: Vec<_> = streams
          .iter()
          .map(|(id, stream)| {
            serde_json::json!({
                "id": id,
  ...
  fn handle_event(&mut self, event: &PluginEvent) -> Option<PluginResponse> {
    ...
    *self
    ...
    if let PluginEvent::PluginLoaded { plugin_id } = event {
      self.plugin_metrics.write().unwrap().insert(
        plugin_id.clone(),
        PluginMetrics {
