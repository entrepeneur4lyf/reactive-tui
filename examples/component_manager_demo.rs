...
use reactive_tui::integration::component_manager::{
  ComponentId, ComponentInstance, ComponentInstanceManager, LifecycleEvent,
};
...
impl Component for TestComponent {
  ...
  fn on_before_unmount(
    ...
    println!(
      "BEFORE_UNMOUNT: Component '{}' preparing to unmount",
      self.name
    );
    let _ = context;
    Ok(())
  ...
  fn on_unmount(&mut self, context: &mut ComponentContext) -> reactive_tui::error::Result<()> {
    println!(
      "UNMOUNTED: Component '{}' unmounted successfully",
      self.name
    );
    let _ = context;
    Ok(())
  ...
  fn on_error(
    &mut self,
    context: &mut ComponentContext,
    error: &reactive_tui::error::TuiError,
  ) -> reactive_tui::error::Result<()> {
    println!(
      "ERROR: Component '{}' encountered error: {}",
      self.name, error
    );
    let _ = context;
    Ok(())
...
async fn main() -> reactive_tui::error::Result<()> {
  ...
  let lifecycle_handler = tokio::spawn(async move {
    while let Some(event) = lifecycle_receiver.recv().await {
      match event {
        ...
        LifecycleEvent::Error(id, error) => {
          println!("LIFECYCLE EVENT: Component {} error: {}", id, error)
        }
      }
    }
  });
  ...
  let instance1 = ComponentInstance::new(ComponentId::new(), Box::new(component1));
  let instance2 = ComponentInstance::new(ComponentId::new(), Box::new(component2));
  let instance3 = ComponentInstance::new(ComponentId::new(), Box::new(component3));
  ...
  if let Some(stats) = manager.get_component_stats(&id1) {
    println!(
      "Component {} stats: render_count={}, mounted={}",
